...
class AllCats(imdb):
    def __init__(self, image_set, year, test=False, devkit_path=None):
        ...
        if not test:
            options = config(section_name='all_categories_train', conf_file='lib/cfgs/detection.cfg')
        else:
            options = config(section_name='all_categories_test', conf_file='lib/cfgs/detection.cfg') 
