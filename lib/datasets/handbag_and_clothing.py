...
class Handbag_And_Clothing(imdb):
    def __init__(self, image_set, year, test=False, devkit_path=None):
        ...
        if not test:
            options = config(section_name='handbag_and_clothing_train', conf_file='lib/cfgs/detection.cfg')
        else:
            options = config(section_name='handbag_and_clothing_test', conf_file='lib/cfgs/detection.cfg')    

        self._classes = eval(options['classes'])
        # self._classes = ('__background__', # always index 0
        #                 'upbody', 'downbody', 'fullbody')
        ...
        self._input_json_file = options['input_json_file']
        self._output_json_file = options['ouput_json_file']
        # self._input_json_file = '/data/home/liuhuawei/clothing_test_sample.json'
        # self._output_json_file = '/data/home/liuhuawei/detection_clothing_test_sample.json'
        # main function to convert json file into xml format
        self._json_to_xml(self._input_json_file)
    ...
    def _json_to_xml(self, json_file):
        ...
        image_set_file = os.path.join(self._data_path, 'ImageSets', 'Main',
                                      self._image_set + '.txt')
        img_to_index = {img_dict['img']:str(idx) for idx, img_dict in enumerate(images)}
        self._index_to_path = {str(idx):img_dict['file_name'] for idx, img_dict in enumerate(images)}
        # print self._index_to_path
        print 'Writing txt file to %s' % image_set_file                                     
        with open(image_set_file, 'w') as f:
            for index in self._index_to_path.keys():
                f.write(str(index)+'\n')        

        # print self._index_to_path.keys()
        # print len(self._index_to_path.keys())          
        # print len(set(self._index_to_path.keys()))
        # raise  
        ...
        for i in xrange(len(annotations)):
            print "Converting json to xml %d/%d" % (i+1, len(annotations))
            annot = annotations[i]
            # img_index = annot['source_id'].split('.')[0]
            img = annot['source_id']            
            category_id = annot['category_id']
            img_id = annot['image_id']
            ## 1. make id to index dict for indexing 
            if img_id not in self._id_to_index.keys():
                self._id_to_index[img_id] = img_to_index[img]
                self._index_to_id[img_to_index[img]] = img_id   
            
            ## 2. make xml file with specified info   
            bbox = np.array([annot['bbox']])
            # abspath = self._index_to_path[img_index]
            abspath = self._index_to_path[img_to_index[img]]
            class_name = self._classes[category_id]
            save_folder = os.path.join(self._data_path, 'Annotations')
            save_name = str(img_to_index[img])

            self._make_xml(abspath, img, class_name, bbox, save_folder, save_name)     
    ...
    def _make_xml(self, img_full_path, img_relative_path, class_name, bbox, save_folder, save_name):      
        ...
        for i in xrange(N):        
            ...
            xmax = str(np.minimum(W, np.float(xmin)+bbox[i, 2]-1))
            ymax = str(np.minimum(H, np.float(ymin)+bbox[i, 3]-1))
        
            obj = doc.createElement("object")        
            name = doc.createElement("name")
            name.appendChild(doc.createTextNode(str(class_name)))
    ...
    ## output director: /output/default/test/  
        ...
        #### we now use coco api to evaluate
   ...
   # def _txt_to_xml(self, txt_file):
   #      infix = '/data/home/liuhuawei/clothing_data/Img/'
   #      image_index = []               
   #      with open(txt_file, 'rb') as f:
   #          lines = f.readlines()
   #          for idx, line in enumerate(lines):
   #              print 'preprocessing %d/%d for xml!!!' % (idx+1, len(lines))
   #              flag = line.strip('\n').split(' ')
   ...
   #              img_relative_path = img_relative_path.replace('/', '_')

   #              image_index.append(img_relative_path)
   #              self._index_to_path[img_relative_path] = img_full_path

   #              class_name = self._classes[category_id]
   #              save_folder = os.path.join(self._data_path, 'Annotations')
   #              save_name = img_relative_path

   #              self._make_xml(img_full_path, img_relative_path, class_name, np.array([bbox]), save_folder, save_name)
