...
###### general parameters 
dataset = 'all_categories_0307'
data_type = 'coco'  #either 'voc' or 'coco'
NET = 'VGG16'  #ether 'VGG16' or 'ResNet101'
num_classes = 12
GPU_ID = 0

###### solver part parameters 
solver_param_base_lr = 0.001
solver_param_stepsize = 260000
solver_param_iter_size = 2
##10 * num_imgaes(for base lr) + 4 * num_images(for base_lr*0.1)
exp_ITERS = 360000
solver_param_snapshot_prefix = '{}_{}_{}_frcnn'.format(NET, dataset, data_type)  

###### Making directorys if necessary
logs_dir = 'experiments/{}/{}/logs'.format(dataset, NET)
scripts_dir = 'experiments/{}/{}/scripts'.format(dataset, NET)
cfgs_dir = 'experiments/{}/{}/cfgs'.format(dataset, NET)
model_dir = 'models/{}/{}/faster_rcnn_end2end'.format(dataset, NET)
...
with open(os.path.join(scripts_dir, 'faster_rcnn_end2end.sh'), 'w') as f:
    f.write('#!/bin/bash\n')
    log_str = '''LOG="experiments/{}/{}/logs/faster_rcnn_end2end_{}_{}.txt.`date +'%Y-%m-%d_%H-%M-%S'`" '''.format(dataset, NET, data_type, NET)
    f.write('set -x\n')
    f.write('set -e\n')
